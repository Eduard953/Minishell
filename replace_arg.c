...
/*   Updated: 2022/02/24 18:12:09 by ebeiline         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	do_this(t_int *n, t_list **arg_arr, char **rep_arg, char *quote, char **line)
{
	if (((*line[n->idx] == '\'' || *line[n->idx] == '\"') && !n->is_q)
		|| (line[n->idx] == quote && n->is_q))
	{
		*rep_arg = ft_strext(rep_arg, line[n->start], (n->idx - n->start));
		quote = line[n->idx];
		n->is_q = !n->is_q;
...
char	**replace_arg(char *line)
{
	...
	while (1)
	{
		do_this(&n, &arg_arr, &rep_arg, &quote, &line);
		if (line[n.idx] == '\0')
		{
